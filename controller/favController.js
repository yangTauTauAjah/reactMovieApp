...
const addMovies = async (req, res) => {
  ...
  try {
    ...
    const exist = await favMovies.findOne({ id: { $in: movies.map(e => e.id) } })
    ...
    await favMovies.insertMany(movies)

    let data = await UserDatas
      .findOneAndUpdate(
        { username },
        { $addToSet: { fav: movies.map(e => e.id) } })
    await data.save()

    return response(res, 201, 'Successfully added movies to favorite list', {})
...
const deleteMovies = async (req, res) => {
  ...
  try {
    ...
    await favMovies.deleteMany({ id: { $in: movies } })
...
const getMovies = async (req, res) => {
  ...
  try {
    ...
    let data = await favMovies.find({ id: { $in: user.fav } }).select('-_id -__v')
